...
const EnhancedRegistration: React.FC<EnhancedRegistrationProps> = ({
  ...
  const handleGoogleSignUp = async () => {
    try {
      ...
      if (error) {
        toast({ title: `❌ ${t('error')}`, description: t('google_login_error') || "Impossible de s'inscrire avec Google.", variant: "destructive" });
      }
    } catch (error) {
          ...
          <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl rounded-2xl shadow-2xl border border-white/20 overflow-hidden">
            ...
            <div className="p-6 sm:p-8 overflow-y-auto max-h-[85vh]">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {/* Google */}
                <Button type="button" variant="outline" className="w-full h-12 border-muted-foreground/20 hover:bg-muted/50 transition-all" onClick={handleGoogleSignUp}>
