      ...
      <Card className="w-full max-w-2xl mx-auto shadow-2xl">
        <CardContent className="p-0">
          <div className={`${slides[currentSlide].bgColor} p-8 sm:p-12 text-center transition-all duration-300`}>
            <div className="mb-6">
              <img src={aquaPilotLogo} alt="AQUA PILOT" className="w-20 h-20 mx-auto mb-4" />
            ...
            <div className="flex justify-center space-x-2 mb-8">
              {slides.map((_, index) => <button key={index} onClick={() => goToSlide(index)} className={`w-3 h-3 rounded-full transition-all duration-200 ${index === currentSlide ? 'bg-aqua-600 scale-125' : 'bg-gray-300 hover:bg-gray-400'}`} />)}
            </div>

            {/* Navigation */}
            <div className="flex justify-between items-center mb-8">
              <Button variant="ghost" onClick={prevSlide} disabled={currentSlide === 0} className="flex items-center gap-2">
              ...
              <Button variant="ghost" onClick={nextSlide} disabled={currentSlide === slides.length - 1} className="flex items-center gap-2">
                {t('next') || 'Suivant'}
            ...
            {currentSlide === slides.length - 1 && <div className="space-y-4">
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  <Button onClick={onRegister} className="bg-gradient-aqua text-white px-8 py-3 text-lg font-semibold hover:opacity-90 transition-opacity">
                    {t('create_account') || 'Créer un compte'}
                  </Button>
                  <Button variant="outline" onClick={onLogin} className="px-8 py-3 text-lg font-semibold border-aqua-600 text-aqua-700 hover:bg-aqua-50">
                    {t('login') || 'Se connecter'}
                  </Button>
                </div>
                <Button variant="ghost" onClick={onComplete} className="text-sm text-gray-500 hover:text-gray-700">
